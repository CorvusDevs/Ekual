...
final class SettingsManager {
    ...
    var shortcutModifiers: UInt {
        didSet {
            UserDefaults.standard.set(shortcutModifiers, forKey: Keys.shortcutModifiers)
        }
    }
    ...
    var shortcutDisplayString: String {
        ...
        parts += Self.keyCodeToString(shortcutKeyCode)
        return parts
    }

    static func keyCodeToString(_ keyCode: UInt16) -> String {
        // Common key codes from Carbon HIToolbox
        switch Int(keyCode) {
        case 0: return "A"
        case 1: return "S"
        case 2: return "D"
    ...
    private init() {
        ...
        if let array = defaults.stringArray(forKey: Keys.excludedBundleIDs) {
            self.excludedBundleIDs = Set(array)
        } else {
            self.excludedBundleIDs = []
        }

        // Load custom shortcut (default: ⌃⌥E)
        if defaults.object(forKey: Keys.shortcutKeyCode) != nil {
            self.shortcutKeyCode = UInt16(defaults.integer(forKey: Keys.shortcutKeyCode))
        } else {
            self.shortcutKeyCode = 14 // kVK_ANSI_E
        }
        if defaults.object(forKey: Keys.shortcutModifiers) != nil {
            self.shortcutModifiers = UInt(defaults.integer(forKey: Keys.shortcutModifiers))
        } else {
            self.shortcutModifiers = NSEvent.ModifierFlags([.control, .option]).rawValue
        }
